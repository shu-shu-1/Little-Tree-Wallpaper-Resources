...
> 🌳 **小树壁纸**（Little Tree Wallpaper）的插件、主题与壁纸源资源索引库。

---

## 📌 用途

本仓库提供**元数据文件**（TOML/JSON 格式）。  
在 v3 规范下，**建议**将可分发文件（例如插件/主题包、图标）统一放入 `src/` 目录，并在元数据中使用**仓库相对路径**引用。
...
位于 `src/` 目录下的代码或可分发文件，依照其对应元数据描述文件中注明的许可协议提供；若元数据引用的是外部项目，则 `src/` 中同步的文件沿用其上游许可证。除非在描述文件中明确声明，这些文件不受 LTR 许可证约束。
